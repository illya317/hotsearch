...
def _filter_by_item_tags(result: dict, tag_list: list[str]) -> dict:
    """Filter results by item-level tags (unified standardized format)."""
    for r in result.get("results", []):
        ...
        data["items"] = [
            item for item in items if any(t in item.get("tags", []) for t in tag_list)
        ]
    return result
...
class APIHandler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, path, params, refresh):
        ...
        if path == "/hotsearch":
            ...
            return self._cached(
                key,
                lambda: TrendsService().collect(
                    sources=["hotsearch"], platform=platform, limit=int(limit)
                ),
                refresh,
            )
        ...
        if path == "/ainews":
            ...
            return self._cached(
                key,
                lambda: TrendsService().collect(
                    sources=["ainews"], source=source, limit=int(limit)
                ),
                refresh,
            )
        ...
        if path == "/github-trending":
            ...
            return self._cached(
                key,
                lambda: TrendsService().collect(sources=["github"], limit=int(limit)),
                refresh,
            )

        # --- unified trends / feeds ---
        if path == "/trends":
            sources = params.get("sources", [None])[0]
            src_list = sources.split(",") if sources else None
            tag = params.get("tag", [None])[0]
            tag_list = tag.split(",") if tag else None
            key = f"trends:{sources or 'all'}:{tag or 'all'}"
            kwargs = {
                k: v[0]
                for k, v in params.items()
                if k not in ("sources", "refresh", "tag")
            }

            def _fetch_and_filter():
                result = TrendsService().collect(sources=src_list, **kwargs)
                if tag_list:
                    result = _filter_by_item_tags(result, tag_list)
                return result

            return self._cached(key, _fetch_and_filter, refresh)

        if path == "/feeds":
            sources = params.get("sources", [None])[0]
            src_list = sources.split(",") if sources else None
            key = f"feeds:{sources or 'all'}"
            kwargs = {
                k: v[0] for k, v in params.items() if k not in ("sources", "refresh")
            }
            return self._cached(
                key, lambda: FeedsService().collect(sources=src_list, **kwargs), refresh
            )

        # --- feeds (legacy endpoints) ---
        if path == "/videos":
            key = "videos"
            return self._cached(
                key, lambda: FeedsService().collect(sources=["videos"]), refresh
            )
    ...
    def _cached(self, key, fn, refresh):
        now = time.time()
        if not refresh:
            # L1: memory
            if key in MEM_CACHE:
